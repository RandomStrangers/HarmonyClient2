#ifndef HC_TEXPACKS_H
#define HC_TEXPACKS_H
#include "Bitmap.h"
HC_BEGIN_HEADER
...
HC_VAR extern struct _Atlas2DData {
...
HC_VAR extern struct _Atlas1DData {
...
hc_bool Atlas_TryChange(struct Bitmap* bmp);
...
hc_bool TextureCache_HasAccepted(const hc_string* url);
/* Whether the given URL is in list of denied URLs. */
hc_bool TextureCache_HasDenied(const hc_string* url);
...
HC_API void TexturePack_Extract(const hc_string* url);

typedef hc_result (*DefaultZipCallback)(const hc_string* path);
hc_result TexturePack_ExtractDefault(DefaultZipCallback callback);

struct TextureEntry;
struct TextureEntry {
	const char* filename;
	void (*Callback)(struct Stream* stream, const hc_string* name);
