#ifndef HC_DRAWER2D_H
#define HC_DRAWER2D_H
#include "Bitmap.h"
#include "Constants.h"
HC_BEGIN_HEADER
...
struct FontDesc { void* handle; hc_uint16 size, flags; int height; };
struct DrawTextArgs { hc_string text; struct FontDesc* font; hc_bool useShadow; };
...
HC_VAR extern struct _Drawer2DData {
...
HC_API void Context2D_Alloc(struct Context2D* ctx, int width, int height);
/* Allocates a new context for 2D drawing, using an existing bimap as backing bitmap */
HC_API void Context2D_Wrap(struct Context2D* ctx, struct Bitmap* bmp);
/* Frees/Releases a previously allocatedcontext for 2D drawing */
HC_API void Context2D_Free(struct Context2D* ctx);
/* Creates a texture consisting of the pixels from the backing bitmap of the given 2D context */
HC_API void Context2D_MakeTexture(struct Texture* tex, struct Context2D* ctx);

/* Draws text using the given font at the given coordinates */
HC_API void Context2D_DrawText(struct Context2D* ctx, struct DrawTextArgs* args, int x, int y);
/* Fills the given area using pixels from the source bitmap */
HC_API void Context2D_DrawPixels(struct Context2D* ctx, int x, int y, struct Bitmap* src);
/* Fills the area with the given color */
HC_API void Context2D_Clear(struct Context2D* ctx, BitmapCol color,
							int x, int y, int width, int height);

/* Fills the given area with a simple noisy pattern */
/* Variation determines how 'visible/obvious' the noise is */
HC_API void Gradient_Noise(struct Context2D* ctx, BitmapCol color, int variation,
						   int x, int y, int width, int height);
...
HC_API void Gradient_Vertical(struct Context2D* ctx, BitmapCol a, BitmapCol b,
							  int x, int y, int width, int height);
/* Blends the given area with the given color */
/*  Note that this only blends RGB, A is not blended */
HC_API void Gradient_Blend(struct Context2D* ctx, BitmapCol color, int blend,
						   int x, int y, int width, int height);

/* Returns how wide the given text would be when drawn */
HC_API int Drawer2D_TextWidth(struct DrawTextArgs* args);
/* Returns how tall the given text would be when drawn */
/*  NOTE: Height returned only depends on the font. (see Font_CalcHeight) */
HC_API int Drawer2D_TextHeight(struct DrawTextArgs* args);
...
HC_API void Drawer2D_MakeTextTexture(struct Texture* tex, struct DrawTextArgs* args);
...
hc_bool Drawer2D_IsWhiteColor(char c);
...
static HC_INLINE BitmapCol GetShadowColor(BitmapCol c) {
...
HC_API void Font_Make(struct FontDesc* desc, int size, int flags);
/* Frees an allocated font */
HC_API void Font_Free(struct FontDesc* desc);
