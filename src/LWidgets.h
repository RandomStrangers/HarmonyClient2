#ifndef HC_LWIDGETS_H
#define HC_LWIDGETS_H
#include "Bitmap.h"
#include "Constants.h"
HC_BEGIN_HEADER
...
struct LWidgetVTABLE {
	...
	hc_bool (*KeyDown)(void* widget, int key, hc_bool wasDown, struct InputDevice* device);
	/* Called when key is pressed and this widget is selected. */
	void (*KeyPress)(void* widget, char c);
	/* Called when mouse hovers/moves over this widget. */
	void (*MouseMove)(void* widget, int idx, hc_bool wasOver);
	...
	void (*OnSelect)(void* widget, int idx, hc_bool wasSelected);
	...
	void (*TextChanged)(void* elem, const hc_string* str);
};
...
#define LWidget_Layout \
	...
	hc_bool hovered;         /* Whether this widget is currently being moused over */ \
	hc_bool selected;        /* Whether this widget is last widget to be clicked on */ \
	hc_bool autoSelectable;  /* Whether this widget can get auto selected (e.g. pressing tab) */ \
	hc_bool dirty;           /* Whether this widget needs to be redrawn */ \
	hc_bool opaque;          /* Whether this widget completely obscures background behind it */ \
	hc_uint8 type;           /* Type of this widget */ \
	hc_bool skipsEnter;      /* Whether clicking this widget DOESN'T trigger OnEnterWidget */ \
...
HC_NOINLINE void LButton_Add(void* screen, struct LButton* w, int width, int height, const char* text, 
							LWidgetFunc onClick, const struct LLayout* layouts);
HC_NOINLINE void LButton_SetConst(struct LButton* w, const char* text);
HC_NOINLINE void LButton_DrawBackground(struct Context2D* ctx, int x, int y, int width, int height, hc_bool active);
...
HC_NOINLINE void LCheckbox_Add(void* screen, struct LCheckbox* w, const char* text, 
								LCheckboxChanged onChanged, const struct LLayout* layouts);
HC_NOINLINE void LCheckbox_Set(struct LCheckbox* w, hc_bool value);
...
struct LInput {
	...
	void (*ClipboardFilter)(hc_string* str);
...
HC_NOINLINE void LInput_Add(void* screen, struct LInput* w, int width, const char* hintText, 
							const struct LLayout* layouts);
HC_NOINLINE void LInput_UNSAFE_GetText(struct LInput* w, hc_string* text);
HC_NOINLINE void LInput_SetText(struct LInput* w, const hc_string* text);
HC_NOINLINE void LInput_ClearText(struct LInput* w);

/* Appends a string to the currently entered text */
HC_NOINLINE void LInput_AppendString(struct LInput* w, const hc_string* str);
/* Sets the currently entered text to the given string */
HC_NOINLINE void LInput_SetString(struct LInput* w, const hc_string* str);
#define LINPUT_HEIGHT 30

/* Represents non-interactable text. */
struct LLabel {
	LWidget_Layout
	hc_bool small; /* whether to use 12pt instead of 14pt font size */
	hc_string text;
	char _textBuffer[STRING_SIZE];
};
HC_NOINLINE void LLabel_Add(void* screen, struct LLabel* w, const char* text, 
							const struct LLayout* layouts);
HC_NOINLINE void LLabel_SetText(struct LLabel* w, const hc_string* text);
HC_NOINLINE void LLabel_SetConst(struct LLabel* w, const char* text);
...
HC_NOINLINE void LLine_Add(void* screen, struct LLine* w, int width, 
							const struct LLayout* layouts);
HC_NOINLINE BitmapCol LLine_GetColor(void);
...
HC_NOINLINE void LSlider_Add(void* screen, struct LSlider* w, int width, int height, BitmapCol color, 
							const struct LLayout* layouts);
HC_NOINLINE void LSlider_SetProgress(struct LSlider* w, int progress);
...
struct LTable {
	...
	hc_bool draggingScrollbar; /* Is scrollbar is currently being dragged */
	...
	hc_uint64 _lastClick; /* timestamp of last mouse click on a row */
	int sortingCol;
};
