...
extern const hc_uint8 Hotkeys_LWJGL[256];
struct HotkeyData {
	int textIndex;     /* contents to copy directly into the input bar */
	hc_uint8 trigger;  /* Member of Key enumeration */
	hc_uint8 mods;     /* HotkeyModifiers bitflags */
	hc_uint8 flags;    /* HOTKEY_FLAG flags */
};
...
typedef hc_bool (*BindTriggered)(int key, struct InputDevice* device);
...
HC_API hc_bool KeyBind_IsPressed(InputBind binding);
...
extern hc_uint8 Bind_IsTriggered[BIND_COUNT];
